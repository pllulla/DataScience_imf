...
* Si ambas variables son factor: Diagrama de barras (diagrama mosaico) de x con distribución acumulada para cada nivel de y.
```{r}
...
legend("topleft", legend = c("Cuadrática ~ Lineal", "Otro vector"),
       col = c("darkgreen", "red"),
       lty=c(3, 2),
       lwd = 3,
       title = "Leyenda")
...
```{r}
matrix(1:20, nrow = 5, ncol = 4, byrow = F)
```
...
# Fila 2, columna 1
plot(fact, main = "Diagrama de barras")

# Fila 1, columna 2
plot(my.mat, main = "Representación matriz ")
...
```{r}
pdf("C:/Users/JMMoreno/Desktop/Lineal_y_cuadrática.pdf")
...
```{r}
jpeg("C:/Users/JMMoreno/Desktop/Lineal_y_cuadrática.jpeg")
...
```
```{r}
barplot(normal.values, border = terrain.colors(length(normal.values)))
```
```{r}
barplot(normal.values, border = topo.colors(length(normal.values)))
```

```{r}
barplot(normal.values, border = cm.colors(length(normal.values)))
```
...
```{r}
v <- c(100, -32, 87, -10, 110)

barplot(v, 
        main = "Diagrama de barras", 
        xlab="Componentes", 
        ylab="Valor", 
        col = heat.colors(length(v))
        )
```
...
barplot(v, 
        ...
        xlab="Componentes", 
        ylab="Valor", 
        col = c("blue", "red", "green", "yellow", "tomato")
        )
```
...
abline(v=mean(normal.values), 
       lty = 2, 
       lwd = 3, 
       col = "darkgreen")

abline(v=median(normal.values), 
       lty = 3, 
       lwd = 3, 
       col = "red")
```
...
plot(a,b, main = "A ~ B", las = 1, col = "red")
