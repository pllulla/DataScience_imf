...
La forma de proceder con datasets es la siguiente:

* data = nombre del dataframe
* x = con nomenclatura ~nombre.variable.
* y = con nomenclatura ~nombre.variable.

Además podemos utilizar parámetros para definir el color con el parámetro `color` que recibe también una variable (categórica genera un color por categoría, una variable numérica un gradiente de color).

La anchura del formato de representación del parámetro `type` vendrá dada por el argumento `size` que recibirá una variable y, en función de su escala representará valores más grandes o más pequeños visualmente.
```{r}
fig <- plot_ly(
  data = iris, x = ~Sepal.Width, y = ~Petal.Width,
  type = "scatter",
  color = ~Sepal.Width, size = ~Sepal.Width
)
...
fig <- plot_ly(iris, x = ~Petal.Length, y = ~Petal.Width, 
               text = ~Species, color = ~Species,
               type = 'scatter', mode = 'markers', 
        marker = list(size = ~Petal.Length, opacity = 0.8))

fig <- fig %>% layout(title = 'Longitud y anchura del pétalo',
         xaxis = list(title = "Longitud pétalo"),
         yaxis = list(title = "Anchura pétalo"))
...
fig <- fig %>% add_histogram(x = ~rnorm(500))
fig <- fig %>% add_histogram(x = ~rnorm(500) + 1)
fig <- fig %>% layout(barmode = "stack") # "stack" | "group" | "overlay" | "relative"
...
fig <- fig %>% add_histogram(x = ~rnorm(500))
fig <- fig %>% add_histogram(x = ~rnorm(500) + 1)
fig <- fig %>% layout(barmode = "group") # "stack" | "group" | "overlay" | "relative"
...
fig <- fig %>% add_histogram(x = ~rnorm(500))
fig <- fig %>% add_histogram(x = ~rnorm(500) + 1)
fig <- fig %>% layout(barmode = "relative") # "stack" | "group" | "overlay" | "relative"
