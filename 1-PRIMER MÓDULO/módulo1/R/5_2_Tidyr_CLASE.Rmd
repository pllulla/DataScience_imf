...
Spread(dataframe, columna:key (Categórica), columna:value (Numérica) )
...
colnames(billboard)
```
gather(datafrme, columna a crear (categórica), valor (numérica))
```{r}
...
separate(aux.tidy.billboard, date.entered, 
...
```{r}
new.billboard <- unite(tidy.billboard, "full_song_name", 
                       artist, 
                       track, 
                       sep = ' - ', 
                       remove = F)
